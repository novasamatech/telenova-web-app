import Address from '@/assets/svg/Address.svg';
import Buy from '@/assets/svg/Buy.svg';
import BuySell from '@/assets/svg/BuySell.svg';
import GiftClaimed from '@/assets/svg/GiftClaimed.svg';
import Loader from '@/assets/svg/Loader.svg';
import Novasama from '@/assets/svg/Novasama.svg';
import Receive from '@/assets/svg/Receive.svg';
import ResetPassword from '@/assets/svg/ResetPassword.svg';
import ResetPasswordDone from '@/assets/svg/ResetPasswordDone.svg';
import Sell from '@/assets/svg/Sell.svg';
import Send from '@/assets/svg/Send.svg';
import Star from '@/assets/svg/Star.svg';
import Success from '@/assets/svg/Success.svg';
import User from '@/assets/svg/User.svg';
import ArrowBold from '@/assets/svg/arrows/arrow-bold.svg';
import ArrowUp from '@/assets/svg/arrows/arrow-up.svg';
import ChevronForward from '@/assets/svg/arrows/chevron-forward.svg';
import DOT from '@/assets/svg/assets/DOT.svg';
import KSM from '@/assets/svg/assets/KSM.svg';
import USDT from '@/assets/svg/assets/USDT.svg';
import WND from '@/assets/svg/assets/WND.svg';
import Backup from '@/assets/svg/backup.svg';
import Blind from '@/assets/svg/blind.svg';
import Currency from '@/assets/svg/currency.svg';
import ExclamationMark from '@/assets/svg/exclamationMark.svg';
import Gift from '@/assets/svg/gift.svg';
import Language from '@/assets/svg/language.svg';
import NovaWallet from '@/assets/svg/nova-wallet.svg';
import Pen from '@/assets/svg/pen.svg';
import PendingGift from '@/assets/svg/pendingGift.svg';
import Present from '@/assets/svg/present.svg';
import ScanQr from '@/assets/svg/scan.svg';
import Settings from '@/assets/svg/settings.svg';
import UserBlock from '@/assets/svg/user-block.svg';
import DotWelcome from '@/assets/svg/welcome/Polkadot.svg';
import UserWelcome from '@/assets/svg/welcome/User.svg';
import SuccessWelcome from '@/assets/svg/welcome/success-welcome.svg';
import Welcome from '@/assets/svg/welcome/welcome.svg';

export const AllIcons = {
  DOT,
  WND,
  KSM,
  USDT,
  Settings,
  Send,
  Receive,
  Address,
  User,
  ChevronForward,
  ArrowBold,
  ScanQr,
  Gift,
  Backup,
  Currency,
  Language,
  NovaWallet,
  Welcome,
  Present,
  UserBlock,
  Pen,
  Blind,
  Success,
  Loader,
  Novasama,
  UserWelcome,
  SuccessWelcome,
  DotWelcome,
  Star,
  GiftClaimed,
  PendingGift,
  ExclamationMark,
  Buy,
  Sell,
  BuySell,
  ArrowUp,
  ResetPassword,
  ResetPasswordDone,
};

export type IconNames = keyof typeof AllIcons;
